...
test.afterAll(async ({ request, baseURL }) => {
    // DELETE THE REPO
    const response = await request.delete(`${baseURL}/repos/${USER}/${REPO}`);
    await expect(response.ok()).toBeTruthy();
})

test('Create a bug report', async ({ request, baseURL }) => {
    const newIssue = await request.post(`${baseURL}/repos/${USER}/${REPO}/issues`,{
        data: {
            title: '[Bug] report 1',
    ...
    const issues = await request.get(`${baseURL}/repos/${USER}/${REPO}/issues`);
...
test('should create a feature request', async ({ request, baseURL }) => {
    const newIssue = await request.post(`${baseURL}/repos/${USER}/${REPO}/issues`, {
    data: {
